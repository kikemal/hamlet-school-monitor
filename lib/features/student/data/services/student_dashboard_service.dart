...
class StudentDashboardService extends BaseService {
  ...
  Future<List<StudentHomeworkWithSubmission>> fetchHomework(String studentId) async {
    try {
      ...
      // Create a map of homework submissions for quick lookup
      final submissionsByHomeworkId = <String, HomeworkSubmission>{};
      for (final sub in submissionResponse) {
        final submission = HomeworkSubmission.fromJson(sub);
        submissionsByHomeworkId[submission.homeworkId] = submission;
      }

      // Combine homework with submissions
      final homeworkWithSubmissions = <StudentHomeworkWithSubmission>[];
      for (final hw in homeworkResponse) {
        final homework = Homework.fromJson(hw);
        final submission = submissionsByHomeworkId[homework.id];
        homeworkWithSubmissions.add(
          StudentHomeworkWithSubmission(
            homework: homework,
